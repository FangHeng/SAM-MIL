...
parser.add_argument('--data_pt_dir', type=str, default=None)
...
if __name__ == '__main__':
    print("########## extract features from pt file ##########")
    ...
    for bag_candidate_idx in range(total):
        ...
        features = torch.load(os.path.join(args.data_pt_dir, slide_id + '.pt'))
        features = features.numpy()
        # print('features size: ', features.shape)

        coords = h5py.File(h5_file_path, 'r')['coords'][:]
        # print('coordinates size: ', coords.shape)

        if features.shape[0] != coords.shape[0]:
            print('####### feat_size: ', features.shape[0], 'coords_size: ', coords.shape[0])
            print('####### features size not match coordinates size')

        asset_dict = {'features': features, 'coords': coords}
        save_hdf5(h5_output_path, asset_dict, attr_dict=None, mode='w')

        file = h5py.File(h5_output_path, "r")
