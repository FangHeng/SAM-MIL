...
class TCGADataset_SAM(Dataset):

    def __init__(self, file_name=None, file_label=None, max_patch=-1, root=None, persistence=True, keep_same_psize=0,
                 is_train=False, _type='NSCLC'):
        """
        Args
        :param images:
        :param transform: optional transform to be applied on a sample
        """
        super(TCGADataset_SAM, self).__init__()

        self.patient_name = file_name
        self.patient_label = file_label
        self.max_patch = max_patch
        self.root = root
        self.all_pts = os.listdir(os.path.join(self.root, 'h5_files')) if keep_same_psize else os.listdir(
            os.path.join(self.root, 'pt_files'))
        self.slide_name = []
        self.slide_label = []
        self.is_group_feat = []
        self.relative_area = []
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            with h5py.File(os.path.join(self.root, 'sam_info', os.path.splitext(self.slide_name[idx])[0] + '.h5'), 'r') as h5_file:
                ...
                relative_area = np.array(h5_file['relative_area'])

        return features, int(label), is_group_feat, relative_area



class C16Dataset_SAM(Dataset):
    def __init__(self, file_name, file_label,root,persistence=False,keep_same_psize=0,is_train=False,is_all=False):
        """
        Args
        :param images:
        :param transform: optional transform to be applied on a sample
        """
        super(C16Dataset_SAM, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        return features, label, is_group_feat, relative_area
