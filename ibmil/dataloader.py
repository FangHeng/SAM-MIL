            ...
            for _idx in _ids:
                if persistence:
                    if keep_same_psize:
                        ...
                        self.slide_name.append(crop_wsi(patch_loc=loc,patch_feat=feat,feat_map_size=keep_same_psize,is_train=is_train))
                    else:
    ...
    def __getitem__(self, idx):
        ...
        if self.persistence:
            ...
            if self.keep_same_psize > 0:
                patch = h5py.File(os.path.join(self.root,'h5', self.slide_name[idx] +'.h5'),"r")
                feat,loc = np.array(patch['features']), np.array(patch['coords'])// 512
                features = crop_wsi(patch_loc=loc,patch_feat=feat,feat_map_size=self.keep_same_psize,is_train=self.is_train)
            else:
...
class PCTDataset(Dataset):
    ...
    def __init__(self, file_name, file_label):
        """
        Args
        :param images: path to image director train.txt
        :param labels: path to the file containing images with corresponding labels
        :param transform: optional transform to be applied on a sample
        """
        super(PCTDataset, self).__init__()
        self.file_name = file_name
    ...
    def __getitem__(self, idx):
        ...
        :return: image and its label
        """
        dir_path = "/data/zhangxiaoxian/pneumonia/images"
        
        # file_path = self.csv_file[idx] # ['0', '/home/tangwenhao/zxx/data/pneumonia/images/18007825_20190327', '0']

        patient_path = os.path.join(dir_path, self.file_name[idx])
        label = int(self.file_label[idx])
        images_list=[]

        ## 
        # image_name = os.listdir(patient_path)
        # image_name.sort(key=lambda x:int(x[6:-4]))
        # indices = np.array(np.round(np.linspace(0,len(image_name)-1,60)),dtype=int)
        # image_name = np.array(image_name)[indices]
        # for name in image_name:
        #     image_dir = os.path.join(patient_path,name)
        #     image = imageio.imread(image_dir)
        #     image = np.transpose([image for i in range(3)], [1, 2, 0])  # H*W*C
        #     image = Image.fromarray(image)
        #     image = self.transform(image)
        #     images_list.append(image)
        # bag=torch.stack(images_list)  
        ...
        if num > 60:
            ...
            sample = np.random.choice(num, num, replace=False)
        sample = np.sort(sample)
        new_feat = feat[sample]
        for i in range(len(new_feat)):
            image_dir = os.path.join(patient_path,new_feat[i])
            image = imageio.imread(image_dir)
            image = np.transpose([image for i in range(3)], [1, 2, 0])  # H*W*C
            image = Image.fromarray(image)
            image = self.transform(image)
            images_list.append(image)
        bag=torch.stack(images_list)  # K*C*H*W 

        return bag, label

if __name__ == "__main__":
    random.seed(2021)
    # label_path='/home/zhangxiaoxian/data/TransMIL/label.csv'

    label_path = '/raid/Data/zhangyi/mil/tcga/zft//label.csv'
    ...
    train_set = TCGADataset(train_p[0],train_p[0],root="/raid/Data/zhangyi/mil/tcga/zft/")
    train_loader = DataLoader(train_set, batch_size=1, shuffle=False, num_workers=2,generator=generator)
