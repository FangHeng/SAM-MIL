...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def visWSI(self, vis_level=0, color = (0,255,0), hole_color = (0,0,255), annot_color=(255,0,0), 
        ...
        scale = [1/int(downsample[0]), 1/int(downsample[1])]
    ...
    # def visWSI_before_downsample(self, vis_level=0, color=(0, 255, 0), hole_color=(0, 0, 255), annot_color=(255, 0, 0),
    ...
    def process_contours(self, sam_model, min_mask_region_area, patch_save_dir, segments_save_dir, patch_level=0,
                         patch_size=256, step_size=256, **kwargs):
        print(kwargs)
        save_path_hdf5 = os.path.join(patch_save_dir, str(self.name) + '.h5')
        if sam_model is not None:
            use_sam = True
            save_path_segments = os.path.join(segments_save_dir, str(self.name) + '.pkl')
        else:
            use_sam = False
            save_path_segments = None
        print("Creating patches for: ", self.name, "...", )
        elapsed = time.time()
        ...
        # 遍历每个轮廓
        for contour in self.contours_tissue:
            # 遍历轮廓中的每个点
        ...
        if use_sam:
            ...
            # calculate the ratio of downsample
            ratio_w = math.log(w / 1024, 2) if w > 1024 else 0
            ratio_h = math.log(h / 1024, 2) if h > 1024 else 0

            downsample_ratio = 2 ** math.ceil(max(ratio_w, ratio_h))
            # print('original downsample_ratio: {}'.format(downsample_ratio))

            # patch_merge_scale = 4
            # print('patch_size {},downsample_ratio {},patch_merge_scale {}x'.format(patch_size, downsample_ratio,
            #                                                                        patch_merge_scale))

            selected_vis_level = len(self.level_downsamples) - 1
            for i, (downsample_x, downsample_y) in enumerate(self.level_downsamples):
                if int(downsample_x) > downsample_ratio and int(downsample_y) > downsample_ratio:
                    selected_vis_level = i - 1
                    break
            downsample_ratio = int(downsample_ratio / int(self.level_downsamples[selected_vis_level][0]))
            print('selected_vis_level: {}'.format(selected_vis_level))
            # print('downsample: {}'.format(self.level_downsamples))
            print('downsample_ratio: {}'.format(downsample_ratio))

            # sam_start_x = start_x // int(self.level_downsamples[selected_vis_level][0])
            # sam_start_y = start_y // int(self.level_downsamples[selected_vis_level][1])
            # sam_stop_x = stop_x // int(self.level_downsamples[selected_vis_level][0])
            # sam_stop_y = stop_y // int(self.level_downsamples[selected_vis_level][1])

            print('original: ({}, {}), ({}, {})'.format(min_x, min_y, max_x, max_y))
            # print('downsampled: ({}, {}), ({}, {})'.format(sam_start_x, sam_start_y, sam_stop_x, sam_stop_y))

            img = self.visWSI(top_left=(min_x, min_y),
                              bot_right=(max_x, max_y),
                              vis_level=selected_vis_level,
                              custom_downsample=downsample_ratio,
                              view_slide_only=True)
            img = np.array(img)
            masks = sam_model.generate(img)

            # self.compress_segmentation_with_padding(masks, patch_merge_scale)

            # add image info to masks
            image_info = {
                ...
                # 'patch_merge_scale': patch_merge_scale
            }

            #
            masks.insert(0, image_info)

            filtered_masks = [masks[0]] if masks else []

            # filter masks according to min_mask_region_area
            filtered_masks.extend([mask for mask in masks[1:] if mask['area'] >= min_mask_region_area])

            # timer
            end_sam = time.time()

            # print vis_level & downsample_ratio & time
            print('vis_level: {}, downsample_ratio: {}, time: {:.2f}s'.format(selected_vis_level, downsample_ratio,
                                                                              end_sam - start_sam))
            with open(save_path_segments, 'wb') as file:
                pickle.dump(filtered_masks, file, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            # if use_sam:
            #     asset_dict, attr_dict, filtered_masks = self.process_contour(cont, self.holes_tissue[idx], patch_level,
            #                                                                  save_path_hdf5, save_path_segments,
            #                                                                  patch_size, step_size, use_sam, sam_model,
            #                                                                  min_mask_region_area, **kwargs)
            # else:
            #     asset_dict, attr_dict, _ = self.process_contour(cont, self.holes_tissue[idx], patch_level,
            #                                                     save_path_hdf5, save_path_segments,
            #                                                     patch_size, step_size, use_sam, sam_model,
            #                                                     min_mask_region_area, **kwargs)
            asset_dict, attr_dict = self.process_contour(cont, self.holes_tissue[idx], patch_level,
                                                            save_path_hdf5,
            ...
            if len(asset_dict) > 0:
                if init:
                    ...
                    #     with open(save_path_segments, 'ab') as file:
                    #         pickle.dump(filtered_masks, file, protocol=pickle.HIGHEST_PROTOCOL)
                    # else:
                    #     save_hdf5(save_path_hdf5, asset_dict, mode='a')
                    save_hdf5(save_path_hdf5, asset_dict, mode='a')
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path_hdf5, patch_size=256, step_size=256,
        ...
        # if use_sam and (w % patch_size != 0 or h % patch_size != 0):
        #     w_remainder = w % patch_size
        #     h_remainder = h % patch_size
        #     w += 0 if w_remainder == 0 else patch_size - w_remainder
        #     h += 0 if h_remainder == 0 else patch_size - h_remainder
        #     print("Adjusted Bounding Box to fit patch size:", start_x, start_y, w, h)
        #     stop_x = start_x + w
        #     stop_y = start_y + h
        ...
        #     # print('original downsample_ratio: {}'.format(downsample_ratio))
        #
        #     patch_merge_scale = (patch_size // downsample_ratio) // 4
        #     print('patch_size {},downsample_ratio {},patch_merge_scale {}x'.format(patch_size,downsample_ratio,patch_merge_scale))
        #
        #     selected_vis_level = len(self.level_downsamples) - 1
        #     for i, (downsample_x, downsample_y) in enumerate(self.level_downsamples):
        #         if int(downsample_x) > downsample_ratio and int(downsample_y) > downsample_ratio:
        ...
        #     # sam_stop_y = stop_y // int(self.level_downsamples[selected_vis_level][1])
        #
        #     print('original: ({}, {}), ({}, {})'.format(start_x, start_y, stop_x, stop_y))
        #     # print('downsampled: ({}, {}), ({}, {})'.format(sam_start_x, sam_start_y, sam_stop_x, sam_stop_y))
        #
        #     img = self.visWSI(top_left=(start_x, start_y),
        #                       bot_right=(stop_x, stop_y),
        #                       vis_level=selected_vis_level,
        #                       custom_downsample=downsample_ratio,
        #                       view_slide_only=True)
        #     img = np.array(img)
        ...
        if len(results) > 0:
            ...
            #             'name': self.name,
            #             'save_path_hdf5': save_path_hdf5,
            #             'save_path_segments': save_path_segments}
            # else:
            #     attr = {'patch_size': patch_size,  # To be considered...
            #             'patch_level': patch_level,
            #             'downsample': self.level_downsamples[patch_level],
    ...
    def compress_segmentation_with_padding(self, mask, patch_merge_scale):
        for mask_item in mask:
            segmentation = np.array(mask_item['segmentation'])
            # 计算新数组的尺寸，向上取整以处理边界情况
            new_height = np.ceil(segmentation.shape[0] / patch_merge_scale).astype(int)
            new_width = np.ceil(segmentation.shape[1] / patch_merge_scale).astype(int)
            # 初始化新的二维数组
            new_segmentation = np.zeros((new_height, new_width), dtype=bool)

            for i in range(new_height):
                for j in range(new_width):
                    # 计算当前块的索引范围，确保不会超出原始数组的边界
                    start_row = i * patch_merge_scale
                    end_row = min((i + 1) * patch_merge_scale, segmentation.shape[0])
                    start_col = j * patch_merge_scale
                    end_col = min((j + 1) * patch_merge_scale, segmentation.shape[1])
                    # 提取块，如果块的尺寸小于patch_merge_scale * patch_merge_scale，则自动填充False
                    block = segmentation[start_row:end_row, start_col:end_col]
                    padded_block = np.pad(block, (
                    (0, patch_merge_scale - block.shape[0]), (0, patch_merge_scale - block.shape[1])), 'constant',
                                          constant_values=False)
                    # 统计True和False的数量，并选择数量较多的值
                    true_count = np.sum(padded_block)
                    # false_count = padded_block.size - true_count
                    new_segmentation[i, j] = true_count > 0

            # 替换原先的segmentation数组
            mask_item['segmentation'] = new_segmentation
